...
LAP_INFO_FONT = pygame.font.SysFont("lucida sans", 42)
...
can_display.insert("Powerup")
can_display.insert("Ready to drive mode active")
for _ in range(20):
    can_display.insert("V1")
while not crashed:
    ...
    gameDisplay.blit(BATT_TMP_FONT.render(str(int(battery_temperature)).rjust(3, " "), True, BATT_TMP_COL), BATT_TMP_POS)
    ...
    # power output
    power_output = 5
    gameDisplay.blit(POWER_OUTPUT_FONT.render(str(int(power_output)).rjust(3, " "), True, POWER_OUTPUT_COL), POWER_OUTPUT_POS)
    gameDisplay.blit(POWER_OUTPUT_UNIT_FONT.render("kW", True, POWER_OUTPUT_COL),
                     (POWER_OUTPUT_POS[0] + 138, POWER_OUTPUT_POS[1] + 21))

    # lap info
    LAP_INFO_SPACING = 50
    gameDisplay.blit(LAP_INFO_FONT.render(str(lap_counter.current_lap).rjust(2, " ") + " / " + str(lap_counter.total_laps) + " laps",
    ...
    gameDisplay.blit(LAP_INFO_FONT.render("LAST " + power_consumption_feedback_total_text + "%",
                                          True, LAP_INFO_COL), (LAP_INFO_POS[0], LAP_INFO_POS[1]+LAP_INFO_SPACING*2))
